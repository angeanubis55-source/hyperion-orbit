...
import { ROCKET_IDS } from '../COMBAT/ROCKET_TYPES.js';
...
const server = spawn(process.execPath, ['SCRIPTS/GAME_SERVER.js'], { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
const serverErrors = [];
...
try {
  ...
  const resource = new URL('NPC/NPC_SPRITES/STREUNER/1.png', url);
  ...
  const optionalRocketPaths = new Set(ROCKET_IDS.map(id => `/COMBAT/MUNITIONS/${id.toUpperCase()}.png`));
  ...
  const boot = async label => {
    ...
    assert.equal(requests.includes('/ASSETS_MANIFEST.json'), false, 'Global asset manifest must not be fetched');
    assert.equal(new Set(requests.filter(p => /^\/NPC\/NPC_SPRITES\/STREUNER\/\d+\.png$/.test(p))).size, 32, 'All original Streuner frames must be prepared');
    assert.equal(requests.some(p => /\/NPC\/NPC_SPRITES\/STREUNER\/sheet\./.test(p)), false);
  ...
  assert.equal(requests.includes('/ASSETS_MANIFEST.json'), false);
  ...
  await mkdir('TEST/REPORTS/sector-loading', { recursive: true });
  await writeFile(`TEST/REPORTS/sector-loading/${networkName === 'local' ? 'results' : networkName}.json`, JSON.stringify(results, null, 2) + '\n');
