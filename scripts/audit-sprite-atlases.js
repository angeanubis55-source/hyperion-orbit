// Usage: node scripts/audit-sprite-atlases.js CANDIDATE_DIRECTORY
...
import { SHIP_EFFECTS } from '../src/data/shipEffects.js';
...
const output = resolve(root, process.argv[3] || 'reports/sprite-atlas-audit');
  ...
  while (cursor < inventory.groups.length) {
    ...
    try {
      const result = await page.evaluate(async pages => {
        const { createImageLoader } = await import('/src/core/ImageLoader.js');
