    ...
    try {
      await page.addInitScript(({ coldStart }) => {
        ...
        CanvasRenderingContext2D.prototype.drawImage = function(img, ...args) {
          if (img?.src?.includes('/Pet/')) window.__petLayers.push(img.src);
          return originalDraw.call(this, img, ...args);
        };
        const user = {
          id: "smoke-user",
          pseudo: "Smoke",
          email: "smoke@local",
          password: "test",
          ship: "PhoenixBleu", pet: { owned: true, active: true, exp: 39000000, hp: 210000, fuel: 50000 },
      ...
      console.log('PET layers', await page.evaluate(() => [...new Set(window.__petLayers)]));
      await page.screenshot({path:'reports/pet-in-game.png'});
